...
type Action = 'candidate' | 'topic' | 'alias' | 'implication' | 'merge' | 'reverse_merge' | 'assignment';
...
async function version(deps: GovernanceDependencies, type: string, resourceId: string): Promise<number | null> {
  const rows = await deps.runQuery('SELECT version FROM topic_governance_versions WHERE resource_type=? AND resource_id=?', [type, resourceId]);
  return rows[0] ? Number(rows[0].version) : 0;
}
const bump = (type: string, resourceId: string) => ({sql:"INSERT INTO topic_governance_versions (resource_type,resource_id,version) VALUES (?,?,1) ON CONFLICT(resource_type,resource_id) DO UPDATE SET version=version+1",params:[type,resourceId]});
...
export async function handleTopicGovernance(request: TopicGovernanceRequest, deps: GovernanceDependencies, cookie: string | null, email: string): Promise<GovernanceResult> {
  ...
  if (request.action === 'candidate') {
    ...
    const v = await version(deps, 'candidate', request.candidateId); if (v !== request.expectedVersion) return stale();
    ...
    statements.push({sql:"UPDATE topic_candidates SET status=?,resolved_topic_id=?,reviewed_at=?,reviewed_by=? WHERE id=?",params:[request.decision === 'approve' ? 'approved' : 'rejected', request.decision === 'approve' ? topicId : null, now,email,request.candidateId]},bump('candidate',request.candidateId));
    if (request.decision === 'approve') statements.push({sql:"UPDATE topics SET status='active',verification_state='published',registry_version=registry_version+1,updated_at=? WHERE id=?",params:[now,topicId]},bump('topic',topicId));
    ...
    const topicId=request.topicId!; const v = await version(deps,'topic',topicId); if (v !== request.expectedVersion) return stale();
    ...
    statements.push({sql:'UPDATE topics SET display_name=?,updated_at=?,registry_version=registry_version+1 WHERE id=?',params:[request.displayName.trim(),now,topicId]},bump('topic',topicId));
    ...
    if (await version(deps,'alias',`${alias}:${request.topicId}`) !== request.expectedVersion) return stale();
    ...
    statements.push({sql:"INSERT INTO topic_aliases (normalized_alias,topic_id,alias_type,requires_context,context_rule_json,verification_state,created_at) VALUES (?,?,?,?,?,'verified',?) ON CONFLICT(normalized_alias,topic_id) DO UPDATE SET alias_type=excluded.alias_type,requires_context=excluded.requires_context,context_rule_json=excluded.context_rule_json,verification_state='verified'",params:[alias,request.topicId,request.aliasType,request.requiresContext?1:0,request.requiresContext?request.contextRuleJson:null,now]},bump('alias',`${alias}:${request.topicId}`));
    ...
    if (await version(deps,'implication',`${request.topicId}:${request.targetTopicId}`) !== request.expectedVersion) return stale();
    statements.push({sql:"INSERT INTO topic_implication_rules (source_topic_id,implied_topic_id,required_context_json,maximum_depth,verification_state) VALUES (?,?,?,1,'verified') ON CONFLICT(source_topic_id,implied_topic_id) DO UPDATE SET verification_state='verified'",params:[request.topicId,request.targetTopicId,request.contextRuleJson ?? '{}']},bump('implication',`${request.topicId}:${request.targetTopicId}`));
    ...
    if (!validId(request.topicId)||!validId(request.targetTopicId)||request.topicId===request.targetTopicId) return {success:false,error:'Two distinct valid topics are required.'};
    const v=await version(deps,'topic',request.topicId!); if(v!==request.expectedVersion)return stale();
    ...
    if(request.action==='merge') statements.push({sql:"UPDATE topics SET status='merged',verification_state='verified',replaced_by_topic_id=?,updated_at=?,registry_version=registry_version+1 WHERE id=?",params:[request.targetTopicId,now,request.topicId]},bump('topic',request.topicId!));
    else statements.push({sql:"UPDATE topics SET status='active',replaced_by_topic_id=NULL,updated_at=?,registry_version=registry_version+1 WHERE id=?",params:[now,request.topicId]},bump('topic',request.topicId!));
    ...
    if (await version(deps,'assignment',assignmentKey) !== request.expectedVersion) return stale();
    ...
    if (request.decision === 'reject') {
      ...
      statements.push(audit(request.topicId,request.clusterId,'remove',current,null,email,request.expectedVersion,now),bump('assignment',assignmentKey));
    } else {
      ...
      statements.push(audit(request.topicId,request.clusterId,request.lock?'lock':'assign',current,{role:request.role,locked:Boolean(request.lock)},email,request.expectedVersion,now),bump('assignment',assignmentKey));
    }
