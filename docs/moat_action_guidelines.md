...
* **Status:** ✅ **PHASE 2 COMPLETE (Phase 2.1 – 2.7 Shipped)** — Pillar B's launch scope, its growth pipeline, and both items carried forward from it are live.
  ...
  * Phase 2.7: Wire Mentions matching & growth-signal counter.
    ...
    * **Live browser verification (2026-09-03, post-commit):** the stale Playwright/Chrome session noted above was killed and a fresh one confirmed the feature end-to-end. `#program/tejas-mk1a` renders clickable chips for both linked suppliers (Bharat Electronics Limited, Bharat Dynamics Limited); clicking one opens the correct Supplier Dossier. On the `hal` dossier, the Live Wire Mentions tab correctly surfaced 2 real currently-loaded stories matched via the `HAL` alias ("Should HAL Build A Stealth Tejas Mk2...", "IAF Prepares to Induct First Tejas Mk1A Squadron..."); on `bel`, it correctly showed the empty state, since no loaded story happens to mention BEL/Bharat Electronics Limited right now — confirming the matcher discriminates real matches from genuine non-matches rather than being silently broken either way. The Ecosystem card grid, coverage strip, and card→dossier click path all render correctly at 390px mobile width. `/api/suppliers/growth` 404s under plain `vite dev` (expected — Pages Functions don't run there) and the coverage strip correctly omits the growth chip without throwing, exactly as the fail-soft design intends. One caveat: a stale service worker cache from an earlier session initially masked the chips entirely (unregistering it and hard-reloading fixed this) — worth remembering if a future manual check "confirms" a Phase 2.4/2.7 regression that a hard refresh makes disappear.
