...
export const STRATEGIC_ACTION_REGEX = /\b(trials?|flight\s+tests?|test\s+fires?|test\s+fired|test\s+firing|user\s+trials?|field\s+trials?|sea\s+trials?|basin\s+trials?|harbou?r\s+trials?|weapon\s+trials?|missile\s+launch(?:es)?|induct(?:ion|ions|ing|ed|s)?|commission(?:ing|ed|s)?|hand(?:ed|s|ing)?\s+over|roll(?:ed|s|ing)?\s+out|maiden\s+flight|first\s+flight|deliver(?:y|ies|ed|ing|s)?(?:\s+of)?|live\s+firings?|live-fire|firing\s+drills?|tactical\s+(?:exercises?|drills?|operations?)|joint\s+exercises?|multilateral\s+exercises?|bilateral\s+exercises?|war\s+games?|combat\s+drills?|airborne\s+(?:assault|drills?|insertions?|operations?)|air\s+combat|scramble(?:s|d|ing)?|intercept(?:ion|ed|s|ing)?|combat\s+readiness|operational\s+readiness|forward\s+(?:operational\s+)?deploy(?:ment|ments|ed|ing|s)?|deployed\s+along|operational\s+patrol|anti-piracy|anti-submarine|counter-terror|counter-insurgency|search\s+and\s+rescue|combat\s+air\s+patrol|surveillance\s+sorties?|special\s+forces|amphibious\s+(?:assault|exercise|operation|landing)|contracts?|aon\s+approv(?:al|als|ed)?|dac\s+approv(?:al|als|ed)?|ccs\s+clear(?:ance|ances|ed)?|rfp\s+issued|contracts?\s+signed|order\s+placed|procurement\s+cleared|capital\s+acquisitions?|transfer\s+of\s+technology|tot|memorandum\s+of\s+understanding|mous?)\b/i;
