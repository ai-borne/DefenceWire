...
/**
 * The sole SQL used to hydrate published canonical memberships into the
 * reader feed: joins `cluster_topics` (which, by the
 * `cluster_topics_only_validated_insert`/`_update` triggers in d1/schema.sql,
 * only ever holds accepted effective memberships) against `topics` filtered
 * to active/published, so a topic downgraded after assignment is excluded
 * even though its `cluster_topics` row was never deleted. Exported so tests
 * can run this exact statement against a real schema rather than a
 * re-typed copy that could silently drift from production behavior.
 */
export function buildPublishedTopicsStatement(clusterIds: string[]): D1Statement {
  const placeholders = clusterIds.map(() => '?').join(',');
  return {
    sql: `SELECT ct.cluster_id, t.id, t.display_name,
      ...
      ORDER BY ct.cluster_id, t.display_priority DESC, t.id ASC`,
    params: clusterIds
  };
}
...
export async function hydratePublishedTopics(
  ...
  const result = await executeD1Query(buildPublishedTopicsStatement(ids), config, fetchFn);
